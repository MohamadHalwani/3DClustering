...
class DepthEstimator
{
  ...
  private:
      ...
      ros::Publisher pointcloud_publisher_ = ros_node_.advertise<sensor_msgs::PointCloud2>("point_cloud_Clustering", 1);
      ...
      ros::Publisher FinalPC_pub = ros_node_.advertise<sensor_msgs::PointCloud2>("pointcloud", 1);
      ...
      void ProcessEvents()
      {
        ...
        // ros::Time t0_, t1_;
        // this->trajectory_.getFirstControlPose(&T0_, &t0_);
        // this->trajectory_.getLastControlPose(&T1_, &t1_);
        // geometry_utils::Transformation T_w_rv;
        // this->trajectory_.getPoseAt(ros::Time(0.5 * (t0_.toSec() + t1_.toSec())), T_w_rv);
        ...
        //this->mapper_.evaluateDSI(this->events_, this->trajectory_ , T_rv_w);
        ...
        pcl::toROSMsg(*mappc_2, *this->ros_pointcloud_);
        ...
        this->geo_.PointsRegistration(registeredPC, holes_pos_intertial);
        ...
        LOG(INFO) << " after publishing holes";
        sleep(1);
        this->plane_quat_orientation_pub.publish(PlaneQuat);
        LOG(INFO) << " after publishing orientatation";
        this->events_.clear();
        this->poses_.clear();   
        this->pose_list_.clear();
        this->pose_timestaps_.clear();
