...
Meanshift::Meanshift(ros::NodeHandle & nh) : nh_(nh) //, ros::NodeHandle nh_private//
{
	...
	//cam_info_subs_ = nh_.subscribe("/dvs/camera_info", 1, &sensor_msgs::CameraInfoCallback, this);
...
if(ClusterID_update_ != prev_GraspID)
{
	iter=0;
...
if (events_size>60)
{
		...
		for (int i=0; i<clustCentX.size();i++)//auto
		{
			...
			//{
				...
				for (int j = beginEvent; j < min(beginEvent+packet, events_size); j++)
				{
					...
					if (dis2<(bandwidth/2)*(bandwidth/2)*240*180*1.5)
					{
	...
	for (int m=0; m<centroid_final.size();m++)
	{
		...
		cv::circle(imageID, cv::Point(cfx,cfy), 1, cv::Scalar(255,255,255), 5, 8, 0);
/* 		for (int n=0;n<events_size;n++)
	...
	if (iter==10) temp_center=centroid_final;
	...
	{ 
		...
		for (int cc=0; cc<temp_center.size(); cc++) // last iteration
		{
			...
			for (int tc=0; tc<centroid_final.size(); tc++) 
			{
				...
				{
					ii++; // find the same center as previous detection
					center_ii.push_back({tcx,tcy}); //center_ii--> 
			...
			if (ii>0)//(center_ii.size()>1) --> find the same center 
			{
				vector_i++; TP++;
			...
			else if (vector_i!=0) // find the repeated centers 
			{
				cluster_center.push_back({ccx+vector_x/vector_i,ccy+vector_y/vector_i});
				//std::cout<<"I find "<< cc <<"th center!!!---dis"<<std::endl;
			}
			else //ii=0, vector_i=0 --> the original center is disappeared in current detection
			{
	...
	for (int w=0;w<cluster_center.size();w++)//centroid_final //cluster_center
	{
		...
		for (int k=0;k<events_size;k++)
		{
			...
			int x_mat=clusters_copy.at<double>(cv::Point(k, 0));
			int y_mat=clusters_copy.at<double>(cv::Point(k, 1));
			...
			{	//points_vector.push_back(points[k]);
				...
				if (w==0)
				{
					...
					if (center_count==1) points_ID0.push_back({cluster_center[w][0],cluster_center[w][1]});
		...
		if (sign) 
		{
			//angle = getOrientation(points_vector, imageID);//cv_image1.image); //orientation in radians //-3.14/2;
			//angle = std::atan2(cluster_center[0][1] - vec_far[0].y, cluster_center[0][0] - vec_far[0].x);
		...
		cv::circle(cv_image.image, cv::Point(cluster_center[0][0],cluster_center[0][1]), 1, cv::Scalar(0,0,255), 5, 8, 0);//--------
